...
def call_train(payload: dict) -> dict:
    """
    POST /train with streaming NDJSON response.
    Displays a progress bar and live epoch log while training.
    Returns the final results dict when done.
    """
    ...
    with requests.post(f"{API_URL}/train", json=payload, stream=True, timeout=3600) as resp:
        ...
        for raw_line in resp.iter_lines():
            ...
            if data.get("done"):
                ...
                line = (
                    ...
                    f"Train  loss {data['tr_loss']:.4f}  acc {data['tr_acc']:.4f}  auc {data['tr_auc']:.4f} │ "
                    f"Val    loss {data['vl_loss']:.4f}  acc {data['vl_acc']:.4f}  auc {data['vl_auc']:.4f}"
                )
                log_lines.append(line)
                status_text.markdown(f"`{line}`")
